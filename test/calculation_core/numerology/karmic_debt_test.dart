  ...
  group('PythagoreanKarmicDebtEngine', () {
    test('detects only canonical compounds with exact provenance', () {
      final findings = PythagoreanKarmicDebtEngine.detect(<KarmicDebtObservation>[
        const KarmicDebtObservation(metric: KarmicDebtMetric.lifePath, compoundValue: 13, reducedValue: 4, provenance: 'life_path.birth_date_digit_sum'),
        const KarmicDebtObservation(metric: KarmicDebtMetric.expression, compoundValue: 14, reducedValue: 5, provenance: 'expression.full_name_value_sum'),
        const KarmicDebtObservation(metric: KarmicDebtMetric.soulUrge, compoundValue: 16, reducedValue: 7, provenance: 'soul_urge.vowel_value_sum'),
        const KarmicDebtObservation(metric: KarmicDebtMetric.birthday, compoundValue: 19, reducedValue: 1, provenance: 'birthday.calendar_day'),
      ]);
    ...
    test('builds observations only from compounds actually seen upstream', () {
      ...
      expect(
        ...
        <KarmicDebtMetric>{KarmicDebtMetric.expression, KarmicDebtMetric.birthday, KarmicDebtMetric.maturity},
      );
      expect(findings.map((item) => item.compoundValue).toSet(), <int>{16, 19, 14});
      expect(
        findings.firstWhere((item) => item.metric == KarmicDebtMetric.expression).provenance,
    ...
    test('builds cycle debt only from an observed cycle compound', () {
      ...
      final observations = PythagoreanKarmicDebtEngine.observationsFromPersonalCycles(cycles);
      final findings = PythagoreanKarmicDebtEngine.detect(observations);
    ...
    test('reduced value alone never invents a Karmic Debt compound', () {
      final findings = PythagoreanKarmicDebtEngine.detect(<KarmicDebtObservation>[
        const KarmicDebtObservation(metric: KarmicDebtMetric.lifePath, compoundValue: 22, reducedValue: 4, provenance: 'life_path.birth_date_digit_sum'),
      ]);
    ...
    test('rejects mismatched compound and reduced values', () {
      expect(
        () => PythagoreanKarmicDebtEngine.detect(<KarmicDebtObservation>[
          const KarmicDebtObservation(metric: KarmicDebtMetric.lifePath, compoundValue: 13, reducedValue: 5, provenance: 'life_path.birth_date_digit_sum'),
        ]),
        throwsStateError,
      );
    });
    ...
    test('rejects duplicate metric observations', () {
      expect(
        () => PythagoreanKarmicDebtEngine.detect(<KarmicDebtObservation>[
          const KarmicDebtObservation(metric: KarmicDebtMetric.birthday, compoundValue: 13, reducedValue: 4, provenance: 'birthday.calendar_day'),
          const KarmicDebtObservation(metric: KarmicDebtMetric.birthday, compoundValue: 14, reducedValue: 5, provenance: 'birthday.calendar_day'),
        ]),
        throwsArgumentError,
      );
    });
    ...
    test('requires non-empty provenance', () {
      expect(
        () => PythagoreanKarmicDebtEngine.detect(<KarmicDebtObservation>[
          const KarmicDebtObservation(metric: KarmicDebtMetric.expression, compoundValue: 14, reducedValue: 5, provenance: '   '),
        ]),
