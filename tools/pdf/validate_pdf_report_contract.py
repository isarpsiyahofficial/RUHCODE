...
contracts = (
    ...
    (DATA_SOURCE, [
        ...
        'requireUiPdfSubjectAndSnapshotParity(', 'UI and PDF subject/snapshot identities do not match.',
    ]),
    (RENDER_SOURCE, [
    ...
    (INSPECTOR_SOURCE, [
        'final class PdfOutputInspector', "RegExp(r'%%EOF\\s*$')", "RegExp(r'/Type\\s*/Catalog\\b')",
    ...
    (DATA_TEST, [
        ...
        'strong UI PDF parity requires subject kind id and snapshot digest',
        'strong UI PDF parity rejects blank UI subject id',
    ]),
    (RENDER_TEST, [
...
if not EVIDENCE.exists():
    ...
    for feature in {
        ...
        'UI and PDF snapshot digest parity can be required explicitly',
        'UI and PDF subject kind plus stable subject ID plus snapshot digest parity can be required together so another client/profile identity cannot reuse the report surface',
        'final EOF plus startxref must resolve to a valid xref table or XRef stream target before return',
...
if not RENDER_EVIDENCE.exists():
    ...
    if set(render_evidence.get('requirements', [])) != {'RC-0950', 'RC-0951', 'RC-0953'}:
        errors.append('local PDF renderer evidence must own exactly RC-0950/0951/0953')
    ...
    for item in {
        ...
        'generated bytes must pass PDF header final-EOF catalog pages-tree page-object and cross-reference structural inspection before return',
        'the Pages-tree declared Count is mandatory and must exactly match the actual Page-object count',
        'the final trailer must contain startxref immediately before the final EOF marker',
        'startxref must be an in-range byte offset that points to an xref table or XRef stream object',
        'junk appended after the final EOF marker is rejected',
...
print('Professional PDF planning/data/local-renderer/output/xref/page-count/table/font-provider contract OK (source-level, not DONE).')