...
def main():
    ...
    req(row['requirement_sha256']==c['bindingRequirementSha256'],'RC-0014 binding SHA drift')
    ...
    req('AstroBody.meanNode' not in p.split('_naifTargetByBody',1)[1].split('};',1)[0],'mean node must not be substituted as physical SPK target')
    req('AstroBody.trueNode' not in p.split('_naifTargetByBody',1)[1].split('};',1)[0],'true node must not be substituted as physical SPK target')
