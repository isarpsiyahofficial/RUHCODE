...
for path in (EVIDENCE, UI, PDF_DATA, PDF_SECTION, UI_TEST, PDF_TEST):
    if not path.is_file():
...
if 'PythagoreanNumerologySnapshotEngine.calculate' in ui:
    raise SystemExit('UI presentation adapter must not recalculate numerology')
if 'PythagoreanNumerologySnapshotEngine.calculate' in pdf_section:
    raise SystemExit('PDF section adapter must not recalculate numerology')
