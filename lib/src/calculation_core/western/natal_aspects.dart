...
final class AspectOrbPolicy {
  AspectOrbPolicy({Map<MajorAspect, double>? maximumOrbDegrees})
  ...
  void validate() {
    for (final aspect in MajorAspect.values) {
      ...
      if (orb == null || !orb.isFinite || orb < 0 || orb > 30) {
        throw StateError('Every major aspect requires a finite orb in [0, 30].');
      }
    }
  }
    ...
    for (var i = 0; i < placements.placements.length - 1; i++) {
      ...
      for (var j = i + 1; j < placements.placements.length; j++) {
        ...
        for (final aspect in MajorAspect.values) {
          ...
          if (orb <= policy.forAspect(aspect) && orb < bestOrb) {
            bestAspect = aspect;
            bestOrb = orb;
          }
        }
