...
- Exact `7752a3c1906b0c731bb770e7a1032bbd1424d271` dalgasındaki airplane workflow run `34631134664` için `contract-and-unit` job fiziksel **SUCCESS** verdi; integration test analyzer temiz ve offline core/unit gate yeşil. Emulator/release-launch job son kontrolde queued olduğundan tüm airplane zinciri VERIFIED değildir.

## RC-1369 PDF font/render blocker — fail-closed manifest IMPLEMENTED

Production runtime halen `UnavailablePdfService` kullanır; approved packaged Unicode font binary + hash zinciri olmadan PDF byte rendering açılmaz.

Bu çalıştırmada blocker production kodunda yapılandırılmış release contract'a dönüştürüldü:

- `dac7363d6cb3b242de9404a649b3c06afabca885` — `lib/src/pdf/pdf_font_release_manifest.dart` eklendi. Canonical family `Noto Sans`, license `OFL-1.1`, approved upstream `https://github.com/google/fonts`, canonical asset paths `assets/fonts/pdf/NotoSans-Regular.ttf` + `NotoSans-Bold.ttf` olarak kilitlendi. SHA-256 alanları ve `binariesPackaged=false` bilerek boş/false tutulur; exact binary/hash yokken `isReleaseReady=false` fail-closed kalır.
- `37b0c482682f22c51562b2398e974f2427c06d0c` — unit contract test eklendi; manifestin packaged binary/hash olmadan release-ready olamayacağını ve source/license/asset-path provenance'ını doğrular.
- GitHub connector binary/non-UTF8 dosya yazamadığı için font blob'ları bu çalıştırmada güvenilir biçimde commit edilemedi. Fake font, placeholder veya hash gevşetmesi yapılmadı.
- Yeni exact head dalgasında `Professional PDF Contract` run `34652368574`, `PDF Structural Contract` `34652368757`, `Flutter Quality` `34652367560`, `RC1362-RC1374 Airplane Mode` `34652367365` dahil ilgili workflow'lar oluşturuldu; son kontrolde queued/pending olduğundan yeni manifest TESTED/VERIFIED yükseltilmedi.

RC-1369 ancak exact approved font binary'leri repo asset'i olduğunda, SHA-256 pinned olduğunda, `PdfAssetFontBundleProvider + LocalPdfService` default production runtime'a bağlandığında, offline Android PDF generation fiziksel SUCCESS verdiğinde ve sonrasında exact-release application/UI yolu kanıtlandığında yükselebilir.
...
1. Exact `37b0c482682f22c51562b2398e974f2427c06d0c` dalgasındaki Flutter Quality/PDF/Airplane workflow sonuçlarını fiziksel doğrula; kırmızıysa root cause'u düzelt.
2. Approved Noto Sans Regular/Bold binary'lerini exact source/license ile repository asset'i olarak ekle; gerçek SHA-256'ları manifestte pinle, `pubspec.yaml` asset zincirine ekle ve runtime'ı `LocalPdfService` + `PdfAssetFontBundleProvider` üzerinden aktive et. Binary commit aracı yoksa requirement'ı gevşetme.
3. RC-1369 production PDF generation'ı Android airplane harness'e 10. capability olarak ekle; integration evidence yine exact-release E2E yerine sayılmasın.
